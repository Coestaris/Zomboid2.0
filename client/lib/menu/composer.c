        ...
        font_t* font, double fontSize, color_t fontColor, int tex, void (*func1)(void), void (*func2)(void))
{
    ...
    menu_container_child_t* button1 = createButton(
            vec_add(vec(w / 2.0, h / 2.0), vec(btnTexture->width / 2.0 + 10, winTexture->height / 4.0)), s1, font, fontSize, fontColor, btnTex,
            composer_func_closeContainer);
    ...
    menu_container_child_t* button2 = createButton(
            vec_add(vec(w / 2.0, h / 2.0), vec(- btnTexture->width / 2.0 - 10, winTexture->height / 4.0)), s2, font, fontSize, fontColor, btnTex,
            composer_func_closeContainer);
        ...
        int tex, void (*func1)(char*), void (*func2)(char*))
{
    ...
    menu_container_child_t* button1 = createButton(
            vec_add(vec(w / 2.0, h / 2.0), vec(btnTexture->width / 2.0 + 12, winTexture->height / 3.5 + 15)), s1, font, fontSize, fontColor, btnTex,
            composer_func_closeContainer);
    button1->object->depth = parent->depth + 3;
    ((button_data_t*)button1->object->data)->wrappedFunc = func1;

    menu_container_child_t* button2 = createButton(
            vec_add(vec(w / 2.0, h / 2.0), vec(- btnTexture->width / 2.0 - 12, winTexture->height / 3.5 + 15)), s2, font, fontSize, fontColor, btnTex,
            composer_func_closeContainer);
    button2->object->depth = parent->depth + 3;
    ((button_data_t*)button2->object->data)->wrappedFunc = func2;
